...
function [ correspondence , T ] = HCode_NICP(Refer_cloud,Match_cloud,Refer_Normal,Match_Normal,weight)
  ...
  %% 先进行初始变换    
    ...
    while( b'*P*b > 0.000000006 )   %迭代计算开始
    ...
    b = zeros(6,1); % 定义 J * e 
        ...
        for i = 1 : Match_cloud_l
            ...
            J = [  eye(3)   ,  -ToAntisymmetric_Mat( Match_cloud(i,1:3)' );
                   zeros(3) ,   ToAntisymmetric_Mat( Match_Normal(i,1:3)') ];%-ToAntisymmetric_Mat( Refer_Normal( correspondence(i,1) , 1:3 )' ) * ToAntisymmetric_Mat( Match_Normal(i,1:3)') ]; 
%             
%             J = [  eye(3)   ,  -ToAntisymmetric_Mat( Match_cloud(i,1:3)' );
%                    zeros(3) ,  -ToAntisymmetric_Mat( Refer_Normal( correspondence(i,1) , 1:3 )' ) * ToAntisymmetric_Mat( Match_Normal(i,1:3)') ]; 
            ...
            b = b + J' * P * [ ( Match_cloud(i,1:3)' - Refer_cloud( correspondence(i,1) , 1:3)' ); (Refer_Normal( correspondence(i,1) ,1:3)' - Match_Normal(i,1:3)')];%( ToAntisymmetric_Mat( Refer_Normal( correspondence(i,1) ,1:3)' ) * Match_Normal(i,1:3)')];
%             b = b + J' * P * [ ( Match_cloud(i,1:3)' - Refer_cloud( correspondence(i,1) , 1:3)' ); ( ToAntisymmetric_Mat( Refer_Normal( correspondence(i,1) ,1:3)' ) * Match_Normal(i,1:3)')];
        ...
        Match_cloud = Trans( Match_cloud , deltT );
        Match_Normal = Trans( Match_Normal , deltT );
%         cloud_2_ = Trans( cloud_2_ , deltT );        
        
%         close all
%         pcshow(Match_cloud , [0,1,0] , 'MarkerSize' , 20);  
%         hold on 
%         quiver3(Match_cloud(:,1),Match_cloud(:,2),Match_cloud(:,3),Match_Normal (:,1),Match_Normal (:,2),Match_Normal(:,3));
%         pcshow(Refer_cloud , [1,0,0] , 'MarkerSize' , 20);
%         quiver3(Refer_cloud(:,1),Refer_cloud(:,2),Refer_cloud(:,3),Refer_Normal (:,1),Refer_Normal (:,2),Refer_Normal(:,3));
%         
%         b'*P*b
